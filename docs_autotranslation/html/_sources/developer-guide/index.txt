...
TBD: Armoury components and how they work together. Also include
...
Messaging4Transport can be configured such that the data tree, RPCs, and
...
The ActiveMQ client implementation for AMQP hides the implementation
behind the Java Message Service (JMS) API. ActiveMQ further depends on
`QPid <https://qpid.apache.org/>`__ for its AMQP clients and broker.
Hence, QPid and JMS bundles are introduced as the dependencies to the
odl-messaging4transport feature.

ActiveMQ also has a `Simple (or Streaming) Text Oriented Message
Protocol (STOMP) <http://stomp.github.io/>`__ implementation in the same
way through StompJmsConnectionFactory, a JMS mapping and interface for
STOMP. `OpenWire <http://activemq.apache.org/openwire.html>`__ is a
...
There is also an `MQTT <http://mqtt.org/>`__ implementation in ActiveMQ,
which is slightly different in the implementation from AMQP, STOMP, and
OpenWire implementations, and does not seem to depend on JMS.

Key APIs and Interfaces
-----------------------

Messaging4Transport consists of the messaging4transport feature which
offers an AMQP message-oriented middleware northbound to MD-SAL, as an
alternative to the traditional RESTCONF northbound.

Use Cases
---------

Multiple research and industrial `use
cases <https://wiki.opendaylight.org/view/Messaging4Transport:Use_Cases>`__
...
concepts and novel research publications.

-  **Software-Defined Buildings**

   `Cassowary <http://www.navigators.di.fc.ul.pt/w2/img_auth.php/9/90/Navtalk20151120_Kathiravelu.paper.pdf>`__
   ...
   and actuator network for smart buildings.

-  **Software-Defined Community Networks**

   `CHIEF <http://www.gsd.inesc-id.pt/~pradeeban/SDS2016/IEEE_SDS_16_CHIEF.pdf>`__
   ...
   deployment of SDN for community clouds.
...
The Open vSwitch database (OVSDB) Plugin component for OpenDaylight
implements the OVSDB `RFC 7047 <https://tools.ietf.org/html/rfc7047>`__
...
project to interact with an OVS instance.
...
| The OpenStack integration architecture uses the following
  technologies:
...
The TCPMD5 project delivers the support for providing access to MD5
Signature Option on operating systems which support it in their TCP
stack. The implementation is compliant with
`RFC-2385 <http://tools.ietf.org/html/rfc2385>`__.
...
This project defines a new TCP option for carrying an MD5 digest in a
...
As this project is fairly small, it provides only one feature that
installs and provides all APIs and implementations for this project.
...
isClassSupported0(). These are implemented in tcpmd5\_jni.c. Individual
...
Contains of topoprocessing-api, topoprocessing-spi and
topoprocessing-impl bundles.
...
-  topoprocessing-impl - contains implemented handlers, listeners and
   aggregators
...
We use the term Physical Node for nodes in the underlay and Logical Node
...
The Topology Processing Framework allows for the creation of aggregated
...
augmenting the `topology YANG
model <https://github.com/opendaylight/yangtools/blob/master/model/ietf/ietf-topology/src/main/yang/network-topology%402013-10-21.yang>`__.
...
Aggregation is an operation which creates an aggregated node from two or
...
First, each new underlay node is inserted into proper topology
...
pointers to all *equal* underlay nodes.

**Equality case.**

In this case, if a node doesn’t fulfill the equality condition with any
...
In this case, an overlay node is created for all underlay nodes, even
...
Processing of updated underlay nodes depends on whether the target field
has been modified. If this it has been, then:

-  if the underlay node belonged to some overlay node, it is withdrawn
...
-  if the node was a "singleton" node, it is put into overlay node, if
   the condition of equality is fulfilled on the new value of target
   field with some existing underlay node

**Unification case.**

Every underlay node belongs to some overlay node. Either with some other
underlay nodes when they have the same value of target field or
otherwise there is one underlay node in an overlay node.

Remove node
^^^^^^^^^^^

The underlay node is removed from appertaining topology store and from
it’s overlay node if it belongs to one.

**Equality case.**

If there is only one underlay node left in the overlay node, the overlay
node is removed.

**Unification case.**

In this case, it is allowed to have one underlay node in overlay node.
...
creating this topology.

NodeIpFiltration
^^^^^^^^^^^^^^^^

This is filtration is based on IP addresses. If a node’s IP address
fulfills mask set in the value tag of the request, this node is put into
resulting overlay topology.

**Create node.**

For a new node fulfilling IP address mask condition, this node is put
into overlay node later written into datastore.

**Update node.**

If IP address of filtered node has changed to value still fulfilling the
...
overlay node is simply removed.

**Remove node.**

If underlay node is part of some overlay node, the overlay node is
simply removed.

Wrapper, RPC republishing, writing mechanism
--------------------------------------------

During the process of aggregation and filtration, overlay nodes (so
...
grouping of overlay nodes (into wrappers).
...
All RPC underlay nodes are re-registered under their corresponding
wrapper ID. RPCs of underlay nodes (belonging to an overlay node) are
gathered, and registered under ID of their wrapper.

RPC Call
^^^^^^^^

When RPC is called on overlay node, this call is delegated to it’s
...
When a wrapper (containing overlay node(s) with it’s underlay nodes(s))
...
thread makes actual write operation in batch.

Classes relationships
---------------------

[1] TopologyRequestHandler instantiates TopologyWriter, TopologyManager.
...
[3] PhysicalNodes are created for relevant income nodes (those having
node ID)
...
[6] Wrapper is translated into adequate format and written into
Datastore
...
Basic Provider class is TopoProcessingProvider which provides startup
and shutdown methods. Otherwise the framework communicates via requests
and outputs stored in DataStores.
